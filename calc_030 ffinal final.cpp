...
const string declkey = "#"; //gives bad token but works how and why idk i have been looking at the code for days trying to understand
...
double primary(){
	...
	switch (t.kind){
        ...
        case square_root:
            ...
            case power: //i don't know why it is not working as it kinda works but doesn't give result why why
            {
                t = ts.get();
                if (t.kind != '(') error("'(' expected");
                double d = expression();
                ...
                if (t.kind != ',') error("',' expected");
                t = ts.get();
                if (t.kind != number) error("not a number can't power it");
                int i = int(t.value);
                if (i != t.value) error("not an integer can't power it");
                t = ts.get();
                if (t.kind != ')') error("')' expected");
                return my_pow(d,i);
            }
