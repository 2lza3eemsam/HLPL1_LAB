...
// This function does not compile as we are trying to assign values to read-only references because we used const int
...
int main()
{
    ...
    //                 // on function return and no reference to original arguments is possible
    // swap_r(7, 9);   // Does not compile as a reference could not be initialized
    //                 // with an r value (literal) since there is no object that
    //                 // could be referenced.
